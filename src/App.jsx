...
const EXERCISES = {
  ...
  ohp: {
    ...
    equipment: 'Гантели',
    technique: [
      'Гантели на уровне плеч, локти чуть впереди корпуса',
      'Жми вверх по небольшой дуге, в верху гантели почти касаются',
      'Не блокируй локти жёстко в верху',
      'Ягодицы и пресс напряжены — не прогибайся в пояснице',
      'Опускай под контролем, без отбива'
    ],
    mistakes: [
      'Прогиб в пояснице — значит, вес великоват или слабый кор',
      'Разведение локтей в стороны — держи их чуть впереди',
      'Жим из-за головы — не надо, плечевой сустав скажет спасибо'
    ],
    tip: 'Делай стоя, а не сидя — включается кор и это честнее для плеч',
    rest: 90
  },
  curl: {
  ...
  deadlift: {
    name: 'Становая тяга',
    muscles: 'Вся задняя цепь • Ягодицы • Спина',
    equipment: 'Штанга',
    technique: [
      'Штанга над серединой стопы, ноги на ширине таза',
      'Хват чуть шире ног, спина прямая, грудь вперёд',
      'Начинаешь подъём ногами (жми пол), а не спиной',
      'Штанга идёт вплотную вдоль голени и бедра',
      'В верхней точке — полное разгибание, без переразгиба'
    ],
    mistakes: [
      'Круглая поясница — СТОП, сбрось вес немедленно',
      'Штанга далеко от ног — плечо нагрузки на спину растёт',
      'Рывок с пола — сначала натяжение, потом подъём'
    ],
    tip: 'Перед каждым повтором — пауза внизу, новый сетап. Это не кардио',
    rest: 240
  },
  pullup: {
  ...
  bulgarian: {
    ...
    muscles: 'Квадрицепсы • Ягодицы • Стабилизаторы',
    equipment: 'Гантели, скамья',
    technique: [
      'Задняя нога на скамье, передняя в шаге вперёд',
      'Опускайся ровно вниз, переднее бедро до параллели',
      'Колено передней ноги над стопой, не уходит внутрь',
      'Корпус слегка наклонён вперёд (больше ягодиц)',
      'Выпрямляйся через пятку передней ноги'
    ],
    mistakes: [
      'Передняя нога слишком близко — колено перегружено',
      'Опора на заднюю ногу — это не выпады',
      'Провал коленом внутрь — слабые ягодицы'
    ],
    tip: 'Сделай все повторы на одну ногу, потом на вторую. Начинай со слабой',
    rest: 90
  },
  lateral: {
...
const PROGRAMS = {
  ...
  B: {
    ...
    exercises: [
      ...
      { id: 'bulgarian', sets: 3, reps: '10 (на ногу)' },
...
const suggestWeight = (exerciseId, history) => {
  ...
  const targetReps = { squat: 8, bench: 8, deadlift: 5, barbell_row: 10, ohp: 12, pullup: 10, incline_db: 10, rdl: 8, leg_press: 12 }[exerciseId] || 12;
  if (topReps >= targetReps) {
    const increment = ['squat', 'deadlift', 'bench', 'rdl'].includes(exerciseId) ? 2.5 : 1;
